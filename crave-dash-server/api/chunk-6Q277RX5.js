...
var createStripePaymentIntent = async (payload, userEmail) => {
  if (!stripeClient) {
    throw new appError_default(500, "Stripe is not configured on server");
  }
  resolveDeliveryAddress(payload);
  const cartItems = await getCartItemsOrThrow(userEmail);
  const subtotal = cartItems.reduce((acc, item) => {
    return acc + Number(item.meal.price) * item.quantity;
  }, 0);
  ...
  const paymentIntent = await stripeClient.paymentIntents.create({
    ...
    metadata: {
      userEmail,
      itemCount: String(cartItems.length),
      flow: "CRAVEDASH_CHECKOUT"
    },
    description: `CraveDash checkout for ${userEmail}`
  });
  if (!paymentIntent.client_secret) {
    throw new appError_default(500, "Failed to create Stripe payment intent");
  }
  return {
...
var completeStripeOrder = async (payload, userEmail) => {
  if (!stripeClient) {
    throw new appError_default(500, "Stripe is not configured on server");
  }
  const paymentIntentId = payload.paymentIntentId;
  if (!paymentIntentId) {
    throw new appError_default(400, "Stripe payment intent id is required");
  }
  ...
  if (paymentIntent.metadata?.userEmail !== userEmail) {
    throw new appError_default(400, "Invalid payment intent for current user");
  }
  const { fullName, phoneNumber, city, area, streetAddress, deliveryInstructions } = resolveDeliveryAddress(payload);
  const cartItems = await getCartItemsOrThrow(userEmail);
  const subtotal = cartItems.reduce((acc, item) => {
...
var createStripePaymentIntent2 = catchAsync(async (req, res) => {
  const result = await OrderService.createStripePaymentIntent(req.body, req.user.email);
  res.status(200).json({
    success: true,
...
var completeStripeOrder2 = catchAsync(async (req, res) => {
  const result = await OrderService.completeStripeOrder(req.body, req.user.email);
  res.status(200).json({
    success: true,
    message: "Stripe payment completed and order created successfully",
    data: result
  });
});
...
var OrderController = {
  ...
  createStripePaymentIntent: createStripePaymentIntent2,
  completeStripeOrder: completeStripeOrder2,
...
router6.post("/stripe/payment-intent", auth_default(Role.CUSTOMER), OrderController.createStripePaymentIntent);
router6.post("/stripe/complete", auth_default(Role.CUSTOMER), OrderController.completeStripeOrder);
